...
onMounted(async () => {
  ...
  try {
    ...
    if (err.status === 404) {
      ...
      error.value =
        "Erreur de filtre : Le champ 'code' existe-t-il dans PocketBase ?";
    } else {
...
<template>
  <div
    class="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-orange-50 text-[#1a1a1a] font-sans selection:bg-purple-100 overflow-x-hidden relative"
  >
    <div
      class="fixed inset-0 opacity-[0.03] pointer-events-none z-50 bg-[url('https://grainy-gradients.vercel.app/noise.svg')]"
    ></div>

    <div class="relative z-10 max-w-5xl mx-auto px-8 py-20">
      <NuxtLink
        to="/"
        class="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-gray-400 hover:text-purple-600 transition-colors mb-12"
      >
        <svg
      ...
      <div v-else-if="ue" class="animate-fade-in-up">
        <header class="mb-16 text-center md:text-left">
          ...
          >
            <span
              class="inline-block px-4 py-1.5 rounded-full bg-orange-100 text-orange-700 text-xs font-black uppercase tracking-widest border border-orange-200"
            >
              Compétence {{ ue.code }}
            </span>
          </div>
          ...
          <h1
            class="text-5xl md:text-7xl font-black tracking-tighter mb-8 text-zinc-900"
          >
            {{ ue.name }}
          </h1>
          <div
            class="h-2 w-24 bg-gradient-to-r from-orange-400 to-pink-500 rounded-full md:mx-0 mx-auto"
          ></div>
        </header>

        <div class="max-w-4xl space-y-12">
          <div
            class="bg-white/60 backdrop-blur-xl p-8 md:p-12 rounded-[2.5rem] border border-white/50 shadow-xl shadow-orange-100/50"
          >
            ...
            <div
              class="description-content text-lg text-gray-600 leading-relaxed"
              v-html="ue.description"
            ></div>
          </div>
        </div>
      </div>
