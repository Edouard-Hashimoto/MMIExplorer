...
onMounted(async () => {
  ...
  const semesterNodes = semestres.map((sem: any) => {
    ...
    const compNodes = competences
      ...
      .map((comp: any) => {
        const specificEns = ensInSem.filter(
          (e: any) => e.competences && e.competences.includes(comp.id)
        );
  ...
  function getBorderColor(d: any) {
    ...
    if (!d.assignedColor) {
      d.assignedColor =
        d.type === "main"
          ? "#ef4444"
          : colorScale(parentColors.length.toString());
  ...
  function updateGraph() {
    const nodeId = (ref: any) =>
      ref && typeof ref === "object" ? ref.id : ref;
  ...
  function handleNodeClick(d: any) {
    ...
    if (isExpanding) {
      ...
      data.nodes.forEach((n) => {
        if (idsToRemove.includes(n.id)) n.expanded = false;
      });
...
<template>
  <div class="page-background">
    <NuxtLink to="/" class="fixed-back-btn"> ← Retour à l'accueil </NuxtLink>
